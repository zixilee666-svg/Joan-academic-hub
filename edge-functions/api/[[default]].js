...
// ============================================================
// 路由处理器 - Zotero 数据种子（GitHub 后门，绕过 eo_token）
// ============================================================

async function handleSeedZotero(request, JWT_SECRET, env) {
  // 验证 SEED_TOKEN（从环境变量读取，GitHub Secrets → EdgeOne 部署）
  const seedToken = request.headers.get('X-Seed-Token') || request.headers.get('x-seed-token');
  const expectedToken = env.SEED_TOKEN || 'zotero-seed-default-token-change-me';

  if (!seedToken || seedToken !== expectedToken) {
    return apiError('Invalid seed token', 401, 'UNAUTHORIZED', request);
  }

  try {
    const admin = await kvGetJson('users:admin');
    if (!admin) return apiError('Admin user not initialized', 500, 'ADMIN_NOT_FOUND', request);
    const userId = admin.id;
    const username = 'admin';

    // 从同源获取 seed JSON（绕过 eo_token：Edge Function 内部 fetch 不受外部保护层限制）
    const originUrl = new URL(request.url);
    const jsonUrl = `${originUrl.protocol}//${originUrl.host}/zotero_full_export.json`;
    console.log('[SeedZotero] Fetching seed data from:', jsonUrl);

    const seedResp = await fetch(jsonUrl);
    if (!seedResp.ok) {
      return apiError('Seed data fetch failed: HTTP ' + seedResp.status, 500, 'SEED_FETCH_ERROR', request);
    }

    const seed = await seedResp.json();
    const { papers, collections } = seed;
    if (!papers || !Array.isArray(papers)) {
      return apiError('Seed data format invalid: papers array expected', 500, 'SEED_INVALID', request);
    }

    // ─── 第一步：创建分类库 ───
    const collectionToLibId = {};
    const existingLibs = await kvGetJson('users:' + userId + ':libraries') || [];
    let libsCreated = 0;

    for (const [zKey, zCol] of Object.entries(collections)) {
      const libName = zCol.name;
      let existing = false;

      for (const libId of existingLibs) {
        const lib = await kvGetJson('libraries:' + libId);
        if (lib && lib.name === libName) {
          collectionToLibId[libName] = libId;
          existing = true;
          console.log('[SeedZotero] Library exists:', libName, libId);
          break;
        }
      }
      ...
      if (!existing) {
        ...
        const lib = {
          ...
          username,
          name: libName,
          description: `Zotero 导入 · ${libName} (${zCol.count} 篇文献)`,
          tags: ['zotero-seed', libName.toLowerCase()],
          papers: [],
          paperCount: 0,
          createdAt: new Date().toISOString(),
          updatedAt: new Date().toISOString()
        ...
        await kvSetJson('libraries:' + libId, lib);
        await kvListAdd('users:' + userId + ':libraries', libId);
        existingLibs.push(libId);
        collectionToLibId[libName] = libId;
        libsCreated++;
        console.log('[SeedZotero] Created library:', libName, libId);
      }
    }
    ...
    let paperSkipped = 0;
    let notesCreated = 0;
    const existingPapers = await kvGetJson('users:' + userId + ':papers') || [];
    // 构建已有 zoteroKey 索引（O(1) 查重）
    const existingZoteroKeys = new Set();
    for (const pid of existingPapers) {
      const p = await kvGetJson('papers:' + pid);
      if (p && p.zoteroKey) existingZoteroKeys.add(p.zoteroKey);
    }

    for (const paper of papers) {
      const { title, authors, year, venue, abstract, doi, url, tags, notes, collections: paperCols, zotero_key, item_type, date_added } = paper;

      // 跳过测试条目
      if (!title || title === 'Test Entry') continue;

      // 按 zotero_key 去重
      if (existingZoteroKeys.has(zotero_key)) {
        paperSkipped++;
        continue;
      }
      ...
      const paperDoc = {
        ...
        itemType: item_type || 'journalArticle',
        dateAdded: date_added || new Date().toISOString(),
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString()
      };

      // 写入论文到 KV
      await kvSetJson('papers:' + paperId, paperDoc);
      await kvListAdd('users:' + userId + ':papers', paperId);
      existingZoteroKeys.add(zotero_key);
      paperCreated++;
      ...
      if (notes && Array.isArray(notes)) {
        ...
        for (const note of notes) {
          ...
          const noteDoc = {
            ...
            content: note.note || '',
            zoteroKey: note.key || '',
            createdAt: note.dateAdded || new Date().toISOString(),
            updatedAt: note.dateModified || new Date().toISOString()
          };
          await kvSetJson('notes:' + noteId, noteDoc);
          paperNoteIds.push(noteId);
          notesCreated++;
        }
        // 关联笔记到论文
        if (paperNoteIds.length > 0) {
          paperDoc.notes = paperNoteIds;
          await kvSetJson('papers:' + paperId, paperDoc);
        }
      }

      // ─── 关联论文到分类库 ───
      if (paperCols && Array.isArray(paperCols)) {
        for (const colName of paperCols) {
          const libId = collectionToLibId[colName];
          if (libId) {
            const lib = await kvGetJson('libraries:' + libId);
            if (lib && !lib.papers.includes(paperId)) {
              lib.papers.push(paperId);
              lib.paperCount = lib.papers.length;
              await kvSetJson('libraries:' + libId, lib);
            }
          }
        }
    ...
    const space = await kvGetJson('spaces:' + username);
    if (space) {
      space.stats.papers = (space.stats.papers || 0) + paperCreated;
      space.stats.libraries = (space.stats.libraries || 0) + libsCreated;
      await kvSetJson('spaces:' + username, space);
    }

    console.log(`[SeedZotero] Done: ${paperCreated} papers, ${paperSkipped} skipped, ${notesCreated} notes, ${libsCreated} new libraries`);

    return success({
      message: 'Zotero seed completed successfully',
      libraries: Object.keys(collectionToLibId),
      papersCreated: paperCreated,
      papersSkipped: paperSkipped,
      notesCreated: notesCreated,
      librariesCreated: libsCreated
    }, 'Seed completed', request);

  } catch (e) {
    console.error('[SeedZotero] Error:', e.message || e);
    return apiError('Seed failed: ' + (e.message || 'unknown'), 500, 'SEED_ERROR', request);
  }
}
...
export async function onRequest(context) {
  ...
  // ========================================
  if (segments[0] === 'admin') {
    // POST /admin/seed-zotero — GitHub 后门：批量导入 Zotero 数据到 KV
    if (segments[1] === 'seed-zotero' && request.method === 'POST') {
      return handleSeedZotero(request, JWT_SECRET, env);
    }
