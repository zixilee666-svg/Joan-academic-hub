...
export async function onRequest(context) {
  ...
  if (segments[0] === 'ai') {
    ...
    // SAFETY-NET: /api/ai/chat 和 /api/ai/parse-paper 由 Cloud Function 接管
    // (pages.config.json regex 已排除 /api/ai/* 路由，正常情况下不会到达此处)
    // Cloud Function: cloud-functions/api/ai/[[default]].js (Node.js v20, 120s, 外部网络可用)
    if (segments[1] === 'chat' || segments[1] === 'parse-paper') {
      return json({ success: false, error: `AI 端点未正确路由 (到达 Edge Function)。请检查 Cloud Function 部署状态`, code: 'ROUTING_ERROR' }, 503, request);
    }
  }
